...
CzkyccoinTracker::CzkyccoinTracker(std::string strWalletFile)
{
...
CzkyccoinTracker::~CzkyccoinTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzkyccoinTracker::Init()
{
...
bool CzkyccoinTracker::Archive(CMintMeta& meta)
{
...
bool CzkyccoinTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzkyccoinTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzkyccoinTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzkyccoinTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzkyccoinTracker::GetSerialHashes()
{
...
CAmount CzkyccoinTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzkyccoinTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzkyccoinTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzkyccoinTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzkyccoinTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzkyccoinTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzkyccoinTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzkyccoinTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzkyccoinTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzkyccoinTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzkyccoinTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzkyccoinTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzkyccoinTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzkyccoinTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzkyccoinTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzkyccoinTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzkyccoinTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzkyccoin from DB\n", __func__, listDeterministicDB.size());
    }
