...
CzkyccoinWallet::CzkyccoinWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zkyccoin seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZkyccoinSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZkyccoinSeed_deprecated()) {
                    LogPrintf("%s: Updated zkyccoinseed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zkyccoin seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zkyccoin wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzkyccoinWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZkyccoinCount(nCountLastUsed);
    else if (!walletdb.ReadZkyccoinCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzkyccoinWallet::Lock()
{
    seedMaster = 0;
}

void CzkyccoinWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzkyccoinWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZkyccoin(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzkyccoinWallet::LoadMintPoolFromDB()
{
...
void CzkyccoinWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzkyccoinWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzkyccoinWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zkyccoinTracker->HasPubcoinHash(pMint.first)) {
...
bool CzkyccoinWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZkyccoin(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zkyccoinTracker which also adds to database
    pwalletMain->zkyccoinTracker->Add(dMint, true);
...
void CzkyccoinWallet::SeedToZkyccoin(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzkyccoinWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzkyccoinWallet::UpdateCount()
{
    nCountLastUsed++;
    CWalletDB walletdb(strWalletFile);
    walletdb.WriteZkyccoinCount(nCountLastUsed);
...
void CzkyccoinWallet::GenerateDeterministicZkyccoin(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzkyccoinWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzkyccoinWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
