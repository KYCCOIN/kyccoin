...
BOOST_AUTO_TEST_CASE(zerocoin_spend_test)
{
    ...
    cWallet.zkyccoinTracker = unique_ptr<CzkyccoinTracker>(new CzkyccoinTracker(cWallet.strWalletFile));
    ...
    BOOST_CHECK_MESSAGE(receipt.GetStatus() == Zkyccoin_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
    ...
    BOOST_CHECK_MESSAGE(receipt2.GetStatus() == Zkyccoin_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
