...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total kyccoin balance of the wallet \n"
            ...
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in kyccoin/kb\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in kyccoin/kb\n"
    ...
    UniValue zkyccoinObj(UniValue::VOBJ);
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zkyccoinObj.push_back(Pair(to_string(denom), ValueFromAmount(chainActive.Tip()->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"kyccoinaddress\"\n"
            "\nReturn information about the given kyccoin address.\n"

            "\nArguments:\n"
            "1. \"kyccoinaddress\"     (string, required) The kyccoin address to validate\n"

            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"kyccoinaddress\", (string) The kyccoin address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: kyccoin address and we have full public key:
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
        throw runtime_error(
            ...
            "2. \"keys\"       (string, required) A json array of keys which are kyccoin addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) kyccoin address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"kyccoinaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"

            "\nArguments:\n"
            "1. \"kyccoinaddress\"  (string, required) The kyccoin address to use for the signature.\n"
