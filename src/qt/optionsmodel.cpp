...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::kyccoin);
    ...
    if (!settings.contains("nAnonymizekyccoinAmount"))
        settings.setValue("nAnonymizekyccoinAmount", 1000);

    nAnonymizekyccoinAmount = settings.value("nAnonymizekyccoinAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizekyccoinAmount"))
        SoftSetArg("-anonymizekyccoinamount", settings.value("nAnonymizekyccoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in kyccoin.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizekyccoinAmount:
            return QVariant(nAnonymizekyccoinAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizekyccoinAmount:
            nAnonymizekyccoinAmount = value.toInt();
            settings.setValue("nAnonymizekyccoinAmount", nAnonymizekyccoinAmount);
            emit anonymizekyccoinAmountChanged(nAnonymizekyccoinAmount);
            break;
        case CoinControlFeatures:
