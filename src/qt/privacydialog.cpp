...
#include "zkyccoincontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zkyccoin ought to be enough for anybody." - Bill Gates, 2017
    ui->zkyccoinpayAmount->setValidator( new QDoubleValidator(0.0, 14190000.0, 20, this) );
    ...
    ui->labelzkyccoinSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zkyccoinpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzkyccoin_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zkyccoin is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_zkyccoin, true));
        if (!ctx.isValid()) {
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zkyccoin...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zkyccoin in ") +
...
void PrivacyDialog::on_pushButtonSpendzkyccoin_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zkyccoin is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_zkyccoin, true));
        if (!ctx.isValid()) {
            // Unlock wallet was cancelled
            return;
        }
        // Wallet is unlocked now, sedn zkyccoin
        sendzkyccoin();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zkyccoin
    sendzkyccoin();
}

void PrivacyDialog::on_pushButtonZkyccoinControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    ZkyccoinControlDialog* zkyccoinControl = new ZkyccoinControlDialog(this);
    zkyccoinControl->setModel(walletModel);
    zkyccoinControl->exec();
}

void PrivacyDialog::setZkyccoinControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzkyccoinSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzkyccoin()
{
    QSettings settings;
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid kyccoin Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zkyccoinpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zkyccoin is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " kyccoin </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zkyccoin</b>";
    ...
    // use mints from zkyccoin selector if applicable
    vector<CMintMeta> vMintsToFetch;
    vector<CZerocoinMint> vMintsSelected;
    if (!ZkyccoinControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZkyccoinControlDialog::GetSelectedMints();
        ...
        for (auto& meta : vMintsToFetch) {
            if (meta.nVersion < libzerocoin::PrivateCoin::PUBKEY_VERSION) {
                ...
                if (nSecurityLevel < 100) {
                    QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zkyccoin require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
                    ui->TEMintStatus->setPlainText(tr("Failed to spend zkyccoin"));
                    ui->TEMintStatus->repaint();
                    return;
                }
            }
    ...
    if (!fSuccess) {
        if (receipt.GetStatus() == Zkyccoin_SPEND_V1_SEC_LEVEL) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zkyccoin require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
            ui->TEMintStatus->setPlainText(tr("Failed to spend zkyccoin"));
            ui->TEMintStatus->repaint();
            return;
        ...
        const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zkyccoin transaction
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zkyccoin was spent successfully update the addressbook with the label
    ...
    // Clear zkyccoin selector in case it was used
    ZkyccoinControlDialog::setSelectedMints.clear();
    ui->labelzkyccoinSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zkyccoin Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " kyccoin, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zkyccoin Mint");
    ...
    // Clear amount to avoid double spending when accidentally clicking twice
    ui->zkyccoinpayAmount->setText ("0");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zkyccoinTracker->GetMints(false);
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zkyccoin </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zkyccoin "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zkyccoin "));
    ui->labelzkyccoinAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zkyccoin </b> "));
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zkyccoin </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzkyccoinSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    bool fButtonsEnabled =  ui->pushButtonMintzkyccoin->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zkyccoin
        ui->pushButtonMintzkyccoin->setEnabled(false);
        ui->pushButtonMintzkyccoin->setToolTip(tr("zkyccoin is currently disabled due to maintenance."));

        // Spend zkyccoin
        ui->pushButtonSpendzkyccoin->setEnabled(false);
        ui->pushButtonSpendzkyccoin->setToolTip(tr("zkyccoin is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zkyccoin
        ui->pushButtonMintzkyccoin->setEnabled(true);
        ui->pushButtonMintzkyccoin->setToolTip(tr("PrivacyDialog", "Enter an amount of kyccoin to convert to zkyccoin", 0));

        // Spend zkyccoin
        ui->pushButtonSpendzkyccoin->setEnabled(true);
        ui->pushButtonSpendzkyccoin->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}
