...
// Copyright (c) 2017-2018 The kyccoin developers
// Distributed under the MIT/X11 software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include "zkyccoincontroldialog.h"
#include "ui_zkyccoincontroldialog.h"
...
std::set<std::string> ZkyccoinControlDialog::setSelectedMints;
std::set<CMintMeta> ZkyccoinControlDialog::setMints;

ZkyccoinControlDialog::ZkyccoinControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZkyccoinControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZkyccoinControlDialog::~ZkyccoinControlDialog()
{
    delete ui;
}

void ZkyccoinControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZkyccoinControlDialog::updateList()
{
...
void ZkyccoinControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZkyccoinControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZkyccoin_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

}

std::vector<CMintMeta> ZkyccoinControlDialog::GetSelectedMints()
{
...
void ZkyccoinControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
