...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZkyccoinStake(spend));
    } else {
        ...
        CkyccoinStake* kyccoinInput = new CkyccoinStake();
        kyccoinInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(kyccoinInput);
    }
