...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT1000IFMN = 3,
    ONLY_NONDENOMINATED_NOT1000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 kyccoinat the same time
    ONLY_1000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zkyccoinsend
enum ZerocoinSpendStatus {
    Zkyccoin_SPEND_OKAY = 0,                            // No error
    Zkyccoin_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    Zkyccoin_WALLET_LOCKED = 2,                         // Wallet was locked
    Zkyccoin_COMMIT_FAILED = 3,                         // Commit failed, reset status
    Zkyccoin_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    Zkyccoin_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    Zkyccoin_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    Zkyccoin_TRX_CREATE = 7,                            // Everything related to create the transaction
    Zkyccoin_TRX_CHANGE = 8,                            // Everything related to transaction change
    Zkyccoin_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    Zkyccoin_INVALID_COIN = 10,                         // Selected mint coin is not valid
    Zkyccoin_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    Zkyccoin_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    Zkyccoin_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    Zkyccoin_SPENT_USED_Zkyccoin= 14,                      // Coin has already been spend
    Zkyccoin_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    Zkyccoin_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZkyccoinBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZkyccoinOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzkyccoinAuto) const;
    ...
    */
    ...
    CzkyccoinWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzkyccoinTracker> zkyccoinTracker;
    ...
    void setZWallet(CzkyccoinWallet* zwallet)
    {
        zwalletMain = zwallet;
        zkyccoinTracker = std::unique_ptr<CzkyccoinTracker>(new CzkyccoinTracker(strWalletFile));
    }

    CzkyccoinWallet* getZWallet() { return zwalletMain; }
    ...
    void setZkyccoinAutoBackups(bool fEnabled)
    {
    ...
    /** zkyccoinreset */
    boost::signals2::signal<void()> NotifyzkyccoinReset;
